...
/**
 * The individual node of a linked structure that stores CountedElement objects.
...
public class TreeNode<T extends Comparable<T>> {
    ...
    // Attributes
    private CountedElement<T> item = null; // the node item
     ...
     */
    public TreeNode(final CountedElement<T> item) {
	this.item = item;
    }
     ...
     */
    private final void inOrderAux(final TreeNode<T> node, final ArrayList<CountedElement<T>> queue) {
	if (node != null) {
	    this.inOrderAux(node.getLeft(), queue);
	    queue.add(new CountedElement<T>(node.getItem()));
     ...
     */
    public CountedElement<T> getItem() {
	return this.item;
    }
     ...
     */
    public final ArrayList<CountedElement<T>> inOrder() {
	final ArrayList<CountedElement<T>> queue = new ArrayList<>();
	this.inOrderAux(this, queue);
	return queue;
    }

    /**
     * Returns an ArrayList of copies of the CountedElement objects in a linked data
     ...
     */
    public final ArrayList<CountedElement<T>> levelOrder() {
	final ArrayList<CountedElement<T>> items = new ArrayList<>();
	...
	if (this != null) {
		...
		items.add(new CountedElement<T>(node.getItem()));
